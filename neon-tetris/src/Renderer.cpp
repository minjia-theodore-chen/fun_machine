...
void Renderer::writeStringToBuffer(int x, int y, const std::string& str, uint8_t colorCode) {
    if (y < 0 || y >= SCREEN_HEIGHT) return;
    int curX = x;
    for (char ch : str) {
        if (curX < 0 || curX >= SCREEN_WIDTH) break;
        nextChars[y][curX] = ch;
...
void Renderer::writeBlockToBuffer(int x, int y, uint8_t colorCode) {
    if (y < 0 || y >= SCREEN_HEIGHT) return;
    if (x < 0 || x + 1 >= SCREEN_WIDTH) return;
    
    // A square block is drawn using two characters, e.g. "██"
    // Block character: '█' is multibyte in UTF-8, so for raw ASCII terminal, we can use "[]" or "▒▒" or "██"
    // Let's use standard brackets "[]" to draw blocks. It looks very retro-clean and compiles safely.
    nextChars[y][x]     = '[';
    nextChars[y][x + 1] = ']';
...
std::string Renderer::getColorCodeStr(uint8_t colorCode) const {
    switch (colorCode) {
        case 1:  return NeonColors::CYAN;
        case 2:  return NeonColors::YELLOW;
        case 3:  return NeonColors::PURPLE;
        case 4:  return NeonColors::GREEN;
        case 5:  return NeonColors::MAGENTA;
        case 6:  return NeonColors::BLUE;
        case 7:  return NeonColors::ORANGE;
        case 8:  return NeonColors::DARK_GRAY;
        case 9:  return NeonColors::TEXT_WHITE;
        default: return NeonColors::RESET;
    }
}
...
void Renderer::draw(const Board& board, const Tetromino& activePiece, const Tetromino& nextPiece, int score, int level, int lines, bool paused, bool gameOver) {
    // 1. Draw Title
    ...
    if (!gameOver) {
        ...
        for (const auto& block : blocks) {
            if (block.y >= 0 && block.y < BOARD_HEIGHT && block.x >= 0 && block.x < BOARD_WIDTH) {
                writeBlockToBuffer(2 + 2 * block.x, 2 + block.y, shapeToColorCode(activePiece.getShape()));
            }
        }
    }
    ...
    if (!gameOver) {
        ...
        for (const auto& b : nextBlocks) {
            writeBlockToBuffer(startCol + 2 * (b.x - minX), 4 + b.y, shapeToColorCode(nextPiece.getShape()));
        }
    }

    // 6. Draw Dashboard/HUD
    writeStringToBuffer(26, 9,  "SCORE: " + std::to_string(score), 2); // Yellow Score
    writeStringToBuffer(26, 11, "LEVEL: " + std::to_string(level), 4); // Green Level
    writeStringToBuffer(26, 13, "LINES: " + std::to_string(lines), 7); // Orange Lines
    ...
    for (int y = 0; y < SCREEN_HEIGHT; ++y) {
        for (int x = 0; x < SCREEN_WIDTH; ++x) {
            ...
            if (nc != currentChars[y][x] || ncol != currentColors[y][x]) {
                ...
                // If it is adjacent to previous output, terminal cursor moves automatically!
                // So we only generate explicit move instructions if we skipped characters.
                if (!cursorMoved) {
                    ss << "\033[" << (y + 1) << ";" << (x + 1) << "H";
                }
