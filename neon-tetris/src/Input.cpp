...
Input::Input() { enableRawMode(); }

Input::~Input() { disableRawMode(); }
...
char Input::readKey() {
  ...
  if (bytesRead > 0) {
    ...
    if (ch == '\033') {
      ...
      // (arrow key inputs occur within microseconds, so they are grouped in the
      // buffer)
      if (read(STDIN_FILENO, &seq[0], 1) > 0 &&
        ...
        if (seq[0] == '[') {
          switch (seq[1]) {
          case 'A':
            return 'w'; // Up Arrow -> map to Rotate ('w')
          case 'B':
            return 's'; // Down Arrow -> map to Down ('s')
          case 'C':
            return 'd'; // Right Arrow -> map to Right ('d')
          case 'D':
            return 'a'; // Left Arrow -> map to Left ('a')
          }
        }
      }
