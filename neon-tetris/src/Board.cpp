...
bool Board::checkCollision(const Tetromino &tetromino) const {
  return checkCollisionAt(tetromino, tetromino.getPosition(),
                          tetromino.getRotation());
}

bool Board::checkCollisionAt(const Tetromino &tetromino, Point position,
                             int rotation) const {
  std::array<Point, 4> blocks =
      tetromino.getBlockPositionsAt(position, rotation);
  ...
  for (const auto &block : blocks) {
    ...
    // Ceilings (above the screen is allowed for rotations, but not below y=0
    // collisions)
    if (block.y < 0) {
      continue;
    }
