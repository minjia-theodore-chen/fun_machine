...
public:
  ...
  // Renders the board grid, the active tetromino, and the next tetromino
  // preview
  void draw(const Board &board, const Tetromino &activePiece,
            const Tetromino &nextPiece, int score, int level, int lines,
            bool paused, bool gameOver);

  // Compares next buffer with current buffer, writes changes to terminal, and
  // swaps buffers
  void display();
...
private:
  ...
  // To keep it memory-efficient, we store an index to a palette of color
  // strings Color Palette mapping: 0: Reset, 1: Cyan, 2: Yellow, 3: Purple, 4:
  // Green, 5: Magenta, 6: Blue, 7: Orange, 8: Dark Gray, 9: White
  ...
  void writeStringToBuffer(int x, int y, const std::string &str,
                           uint8_t colorCode);
